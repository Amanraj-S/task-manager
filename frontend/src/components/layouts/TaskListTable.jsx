      ...
      <table className='min-w-full'>
        <thead>
          <tr className='text-left'>
            ...
            <th className='py-3 px-4 text-gray-800 font-medium text-[13px] hidden md:table-cell'>Created One</th>
          </tr>
        </thead>
        <tbody>
          {tableData.map((task) => (
            <tr key={task._id} className='border-t border-gray-200'>
              ...
              <td className='py-4 px-4'>
                <span className={`px-2 py-2 text-xs rounded inline-block ${getStatusBadgeColor(task.status)}`}>
                  {task.status}
                </span>
              </td>
              <td className='py-4 px-4'>
